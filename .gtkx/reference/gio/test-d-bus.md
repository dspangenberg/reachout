---
description: "A helper class for testing code which uses D-Bus without touching the user’s session bus."
---
...
```tsx
import { GTestDBus } from "@gtkx/jsx/gio";
```
...
`Gio.TestDBusFlags` · default `G_TEST_DBUS_NONE` · construct-only
...
Methods are called on the `Gio.TestDBus` instance, obtained with the `ref` prop or imported from `@gtkx/gi/gio`. Methods inherited from ancestors are documented on their own pages.
