---
description: "Contains the Traffic Class byte of an IPv6 header."
---
...
Traffic Class field to be used in datagrams sent on a `Gio.Socket` use:
```c
...
```tsx
import { GIPv6TclassMessage } from "@gtkx/jsx/gio";
```
...
Methods are called on the `Gio.IPv6TclassMessage` instance, obtained with the `ref` prop or imported from `@gtkx/gi/gio`. Methods inherited from ancestors are documented on their own pages.
