---
description: "Creates widgets for the items taken from a GListModel."
---
...
widget you want to use it with, such as via `Gtk.ListView.setFactory()`.
Reusing factories across different views is allowed, but very uncommon.

```tsx
import { GtkListItemFactory } from "@gtkx/jsx/gtk";
```

## Hierarchy

[GObject](.gtkx/reference/gobject/object.md) → **GtkListItemFactory**

## Props

`ref` receives the `Gtk.ListItemFactory` instance. Every mutable property also has an `onNotify<Prop>` handler prop called with the new value when the property changes. Props inherited from ancestor elements are documented on their own pages.
