---
description: "Uses constraints to describe relations between widgets."
---
...
[GObject](.gtkx/reference/gobject/object.md) → [GtkLayoutManager](.gtkx/reference/gtk/layout-manager.md) → **GtkConstraintLayout**
...
Methods are called on the `Gtk.ConstraintLayout` instance, obtained with the `ref` prop or imported from `@gtkx/gi/gtk`. Methods inherited from ancestors are documented on their own pages.

### `addConstraint`

```ts
addConstraint(constraint: Gtk.Constraint): void
```
...
```ts
addConstraintsFromDescription(lines: string[], hspacing: number, vspacing: number, views: Map<string, Gtk.ConstraintTarget>): Gtk.Constraint[]
```
...
```ts
addGuide(guide: Gtk.ConstraintGuide): void
```
...
```ts
removeGuide(guide: Gtk.ConstraintGuide): void
```
