...
export const visibleTasks = (tasks: Task[], selection: Selection, options: VisibleOptions): Task[] =>
  tasks
    .filter(
      task => inSelection(task, selection) && matchesQuery(task, options.query) && matchesFilter(task, options.filter)
    )
...
export const sidebarCounts = (tasks: Task[], lists: TaskList[]): SidebarCounts => {
  ...
  return {
    ...
    lists: Object.fromEntries(lists.map(list => [list.id, open.filter(task => task.listId === list.id).length]))
  }
}
