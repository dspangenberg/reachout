...
import { PlaceholderSection } from './placeholder-section.js'
...
export const Window = () => {
  ...
  const _lists = useStore(state => state.lists)
  const tasks = useStore(state => state.tasks)
  const _collapsed = useStore(state => state.collapsed)
  const setCollapsed = useStore(state => state.setCollapsed)
  const _showDialog = useStore(state => state.showDialog)
        ...
        <AdwToastOverlay ref={toastOverlayRef}>
          {section === 'tasks' ? (
            ...
            <PlaceholderSection
              section="contacts"
              title={t('Contacts')}
              description={t('Contact management is coming soon')}
              icon="contact-book-symbolic"
              onSelect={setSection}
            />
          ) : (
            <PlaceholderSection
              section="projects"
              title={t('Projects')}
              description={t('Project management is coming soon')}
              icon="kanban-symbolic"
              onSelect={setSection}
            />
          )}
        </AdwToastOverlay>
